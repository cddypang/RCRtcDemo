...
class MainWindow : public QMainWindow, public rcrtc::IRCRTCListener
{
    Q_OBJECT
...
private:  // im rtc callback impl
    ...
    // context: user_token
    friend void rcim_connection_callback_impl(HANDLE context, rc_connection_status code);
    ...
    {
        emit logToPlainText(QString(CUtils::formatSdkResult(code, "Engine internal error!").c_str()));
    }
    ...
    {
        emit logToPlainText(QString(CUtils::formatSdkResult(0, "onKicked, roomId: " + roomId + ", errMsg: " + errMsg).c_str()));
    }
